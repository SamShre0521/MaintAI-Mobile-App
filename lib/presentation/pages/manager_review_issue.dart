...
class _ManagerReviewIssuePageState extends State<ManagerReviewIssuePage> {
  ...
  void initState() {
    ...
    debugPrint('Opened feedback ID: ${widget.feedback.id}');
    debugPrint('Conversation count: ${widget.feedback.conversation.length}');

    for (final message in widget.feedback.conversation) {
      debugPrint('${message.role}: ${message.content}');
    }
  }

  List<ChatMessage> get conversationMessages {
    if (widget.feedback.conversation.isNotEmpty) {
      return widget.feedback.conversation.map((e) {
        return ChatMessage(
  ...
  Widget build(BuildContext context) {
    if (widget.isReadOnly) {
      return _buildScaffold(context, isActionLoading: false);
    }
    ...
    return BlocConsumer<ManagerDashboardBloc, ManagerDashboardState>(
      ...
      builder: (context, state) {
        return _buildScaffold(context, isActionLoading: state.isActionLoading);
      },
    );
  }

  Widget _buildScaffold(BuildContext context, {required bool isActionLoading}) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.isReadOnly ? 'Case Details' : 'Review Issue',
        ...
        actions: [
          Padding(
            ...
            child: _StatusChip(
              ...
              color: _statusColor(widget.feedback.managerStatus),
            ),
          ),
        ],
      ),
      body: ListView(
        padding: EdgeInsets.fromLTRB(16, 10, 16, widget.isReadOnly ? 24 : 160),
        children: [
          ...
          if (widget.isReadOnly &&
              widget.feedback.managerComment.trim().isNotEmpty) ...[
            const SizedBox(height: 14),
            _ReadOnlyCommentCard(comment: widget.feedback.managerComment),
          ],
        ],
      ),
  ...
  Color _statusColor(String status) {
    ...
    if (value == 'approved') return const Color(0xFF22C55E);
    if (value == 'rejected') return const Color(0xFFDC2626);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (feedback.approvedBy != null &&
              feedback.approvedBy!.trim().isNotEmpty) ...[
            const SizedBox(height: 10),
            _CompactInfoRow(
              icon: Icons.verified_user_outlined,
              label: 'Approved By',
              value: feedback.approvedBy!,
            ),
          ],
        ],
      ),
...
class _ReadOnlyCommentCard extends StatelessWidget {
  final String comment;

  const _ReadOnlyCommentCard({required this.comment});
